...
void exchange_ghost_cells(World &world) {
    ...
    std::vector<double> left_line_send(world.latitude);
    std::vector<double> right_line_send(world.latitude);
    std::vector<double> top_line_send(world.longitude);
    std::vector<double> bottom_line_send(world.longitude);
    
    for (uint64_t i = 0; i < world.latitude; ++i) {
        left_line_send[i] = world.data[i*world.longitude + world.longitude-2];
        right_line_send[i] = world.data[i*world.longitude + 1];
    }
    for (uint64_t j = 0; j < world.longitude; ++j) {
        top_line_send[j] = world.data[world.longitude*(world.latitude-2) + j];
        bottom_line_send[j] = world.data[world.longitude + j];
        }
    
    std::vector<double> left_line_recv(world.latitude);
    std::vector<double> right_line_recv(world.latitude);
    std::vector<double> top_line_recv(world.longitude);
    std::vector<double> bottom_line_recv(world.longitude);
    
    int l_neighbor = (mpi_rank + nx - 1) % nx + (mpi_rank / nx) * nx;
    int r_neighbor = (mpi_rank + 1) % nx + (mpi_rank / nx) * nx;
    int t_neighbor = ((mpi_rank / nx + ny - 1) % ny) * nx + mpi_rank % nx;
    int b_neighbor = ((mpi_rank / nx + 1) % ny) * nx + mpi_rank % nx;
    
    
    MPI_Request requests[4];
    
    MPI_Isend(&left_line_send[0], world.latitude, MPI_DOUBLE, l_neighbor, 0, MPI_COMM_WORLD, &requests[0]);
    MPI_Irecv(&left_line_recv[0], world.latitude, MPI_DOUBLE, l_neighbor, 0, MPI_COMM_WORLD, &requests[0]);
    MPI_Isend(&right_line_send[0], world.latitude, MPI_DOUBLE, r_neighbor, 0, MPI_COMM_WORLD, &requests[1]);
    MPI_Irecv(&right_line_recv[0], world.latitude, MPI_DOUBLE, r_neighbor, 0, MPI_COMM_WORLD, &requests[1]);
    MPI_Isend(&top_line_send[0], world.longitude, MPI_DOUBLE, t_neighbor, 0, MPI_COMM_WORLD, &requests[2]);
    MPI_Irecv(&top_line_recv[0], world.longitude, MPI_DOUBLE, t_neighbor, 0, MPI_COMM_WORLD, &requests[2]);
    MPI_Isend(&bottom_line_send[0], world.longitude, MPI_DOUBLE, b_neighbor, 0, MPI_COMM_WORLD, &requests[3]);
    MPI_Irecv(&bottom_line_recv[0], world.longitude, MPI_DOUBLE, b_neighbor, 0, MPI_COMM_WORLD, &requests[3]);
    ...
    for (uint64_t i = 0; i < world.latitude; ++i) {
        world.data[i*world.longitude + 0] = left_line_recv[i];
        world.data[i*world.longitude + world.longitude-1] = right_line_recv[i];
    }
    
    for (uint64_t j = 0; j < world.longitude; ++j) {
        world.data[0*world.longitude + j] = top_line_recv[j];
        world.data[(world.latitude-1)*world.longitude + j] = bottom_line_recv[j];
    }
}
 ...
 */
void simulate(uint64_t num_of_iterations, const std::string& model_filename, const std::string& output_filename, int nx, int ny) {

    // for simplicity, read in full model
    World global_world = read_world_model(model_filename);
    
    int long_div = global_world.longitude / nx;
    int lat_div = global_world.latitude / ny;
    
    // TODO: compute offsets according to rank and domain decomposition
    // figure out size of domain for this rank
    const long int offset_longitude = long_div*(mpi_rank % nx) - 1; // -1 because first cell is a ghostcell
    const long int offset_latitude  = lat_div*(mpi_rank / nx) - 1;
    const uint64_t longitude = long_div + 2; // one ghost cell on each end
    const uint64_t latitude  = lat_div + 2;
    
    /*
    std::cout << "Rank " << mpi_rank << " has longitude " << long_div + 2 << " and lattitude " << lat_div + 2 << "\n";
    std::cout << "Rank " << mpi_rank << " has offset_long " << long_div*(mpi_rank % nx) - 1 << " and offset_lat " << lat_div*(mpi_rank / nx) - 1 << "\n";
    */
    ...
    std::cout << "     " << t_neighbor << "\n";
    std::cout << "     ^\n";
    std::cout << l_neighbor << " <- " << mpi_rank << " -> " << r_neighbor << "\n";
    std::cout << "     v\n";
    std::cout << "     " << b_neighbor << "\n";
    ...
    for (uint64_t iteration=0; iteration < num_of_iterations; ++iteration) {
        ...
        if (mpi_rank == 0) {
            int offset_lat[mpi_size];
            int offset_long[mpi_size];
            std::vector<double> dat(world.longitude*world.latitude*mpi_size);
            
            MPI_Gather(&offset_latitude, 1, MPI_INT, &offset_lat, 1, MPI_INT, 0, MPI_COMM_WORLD);
            MPI_Gather(&offset_longitude, 1, MPI_INT, &offset_long, 1, MPI_INT, 0, MPI_COMM_WORLD);
            MPI_Gather(&world.data[0], world.data.size(), MPI_DOUBLE, &dat[0], world.data.size(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
            ...
            std::vector<double> dat(world.latitude*world.longitude);
            MPI_Recv(&dat[0], longitude*latitude, MPI_DOUBLE, r, r, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            for (uint64_t i = 1; i < latitude-1; ++i)
            for (uint64_t j = 1; j < longitude-1; ++j) {
                uint64_t k_global = (i + offset_lat[r]) * global_world.longitude
                                  + (j + offset_long[r]);
                global_world.data[k_global] = dat[i * longitude + j];
            }
            } */
            ...
            for (int r = 0; r < mpi_size; r++){
                ...
                for (uint64_t j = 1; j < longitude-1; ++j) {
                    uint64_t k_global = (i + offset_lat[r]) * global_world.longitude
                                      + (j + offset_long[r]);
                    global_world.data[k_global] = dat[r*world.longitude*world.latitude + i * longitude + j];
                }
            }
            ...
            if (!output_filename.empty()) {
                ...
                MPI_Gather(&offset_latitude, 1, MPI_INT, NULL, 0, MPI_INT, 0, MPI_COMM_WORLD);
                MPI_Gather(&offset_longitude, 1, MPI_INT, NULL, 0, MPI_INT, 0, MPI_COMM_WORLD);
                MPI_Gather(&world.data[0], world.longitude*world.latitude, MPI_DOUBLE, NULL, 0, MPI_DOUBLE, 0, MPI_COMM_WORLD);
                // MPI_Send(&world.data[0], longitude*latitude, MPI_DOUBLE, 0, mpi_rank, MPI_COMM_WORLD);
            }
        }
...
int main(int argc, char **argv) {
    ...
    for (long unsigned int i = 1; i<argument.size() ; i += 2){
        ...
        if(arg=="-h"){ // Write help
            ...
            nx = std::stoi(argument[i+1]);
        } else if(arg=="--ny"){
            ny = std::stoi(argument[i+1]);
        } else{
    ...
    if (iterations==0)
        throw std::invalid_argument("You must specify the number of iterations "
                                    "(e.g. --iter 10)");
    if (nx*ny != mpi_size)
        throw std::invalid_argument("You must have nx*ny = np "
                                    "(e.g. -np 4 --nx 2 --ny 2)");

    simulate(iterations, model_filename, output_filename, nx, ny);
