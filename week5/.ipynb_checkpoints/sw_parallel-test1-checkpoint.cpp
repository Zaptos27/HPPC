...
void integrate(Water &w, const real_t dt, const real_t dx, const real_t dy, const real_t g) {
    ...
    #pragma acc parallel loop collapse(2) present(w.e, w.u, w.v)
...
void simulate(const Sim_Configuration config) {
    ...
    //integrate called config.iter times

    #pragma acc data copy(water_world,water_history)
    {
...
int main(int argc, char **argv) {
    ...
    std::cout << "This is time when using simulate " << (tend2 - tstart2).count()*1e-9 << "\n";
