...
using real_t = float;
constexpr size_t NX = 8*512, NY = 8*512; //World Size
...
void integrate(Water &w, const real_t dt, const real_t dx, const real_t dy, const real_t g) {
    ...
    #pragma acc parallel loop gang num_gangs(64)
    ...
    #pragma acc parallel loop collapse(2) gang num_gangs(64)
...
void simulate(const Sim_Configuration config) {
    ...
    for (uint64_t t = 0; t < config.iter; ++t) {
        ...
        /*if (t % config.data_period == 0) {
            ...
            water_history.push_back(water_world.e); // This statement does not generate the correct data file for vizualization
        }*/
    }
    }
