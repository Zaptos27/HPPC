...
void master (int nworker, Data& ds) {
  ...
  MPI_Request request[nworker], recived_request[nworker];
  for (i = 0; i < nworker; i++)
  {
    MPI_Isend(&settings[i][0] , 1, MPI_DOUBLE , i+1 , i, MPI_COMM_WORLD , &request[i]);
    MPI_Irecv(&accuracy[i], 1, MPI_DOUBLE, i+1 , i , MPI_COMM_WORLD , &recived_request[i]);
  }
  MPI_Waitall(nworker, request, MPI_STATUS_IGNORE);
  for (i = nworker; i<2*nworker;i++){
    MPI_Isend(&settings[i][0], 1, MPI_DOUBLE, i+1-nworker, i , MPI_COMM_WORLD , &request[i-nworker]);
    MPI_Irecv(&accuracy[i], 1, MPI_DOUBLE, i+1-nworker , i , MPI_COMM_WORLD , &recived_request[i-nworker]);
  }
  int indices[3];
  int index_count;
  while (i < n_settings)
  {
    MPI_Waitsome( nworker , recived_request , &index_count , indices , MPI_STATUS_IGNORE);
    for (int j = 0; j<index_count;j++){
      MPI_Irecv(&accuracy[i], 1, MPI_DOUBLE, indices[j]+1, i, MPI_COMM_WORLD, &recived_request[indices[j]]);
      MPI_Wait(&request[indices[j]] , MPI_STATUS_IGNORE); // confusion
      MPI_Isend(&settings[i][0], 8, MPI_INT, indices[j]+1, i, MPI_COMM_WORLD, &request[indices[j]]);
      i++;
      if(i>=n_settings){
        break;
      }
  ...
  MPI_Waitall(nworker, request , MPI_STATUS_IGNORE);
  MPI_Waitall(nworker, recived_request , MPI_STATUS_IGNORE);
  for (i = 0; i < nworker; i++)
  {
    MPI_Isend(&settings[i][0], 1, MPI_DOUBLE, i+1, NO_MORE_TASKS, MPI_COMM_WORLD, &request[i]);
  }
  MPI_Waitall(nworker, request , MPI_STATUS_IGNORE);
...
void worker(int rank, Data &ds)
{
  ...
  double result;
  std::array<double,8> recived, task;
  MPI_Request request[2];
  MPI_Status status;
  bool first=true;
  MPI_Recv(&task, 8, MPI_DOUBLE, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
  int tag = status.MPI_TAG;
  while (tag != NO_MORE_TASKS)
  {
    MPI_Irecv(&recived , 8 , MPI_DOUBLE , 0 , MPI_ANY_TAG , MPI_COMM_WORLD , &request[1]);
    result = task_function(task, ds);
    ...
    MPI_Isend(&result, 1, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD, &request[0]); 
