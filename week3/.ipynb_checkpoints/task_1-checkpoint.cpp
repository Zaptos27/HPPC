...
void master (int nworker, Data& ds) {
  ...
  while (i < n_settings)
  {
    MPI_Waitany(nworker , recived_request , &index , &status);
    MPI_Irecv(&accuracy[i], 1, MPI_DOUBLE, index+1, i, MPI_COMM_WORLD, &recived_request[index]);
    MPI_Wait(&request[index] , &status); // confusion
    MPI_Isend(&settings[i][0], 8, MPI_DOUBLE, index+1, i, MPI_COMM_WORLD, &request[index]);
    i++;
  }
  MPI_Waitall(nworker, request , MPI_STATUS_IGNORE);
