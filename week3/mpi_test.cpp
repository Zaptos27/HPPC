...
void farmer(int workers)
{
  ...
  for (i = 0; i < workers; i++)
  {
    ...
    MPI_Irecv(&temp, 1, MPI_INT, i+1 , MPI_ANY_TAG , MPI_COMM_WORLD , &recived_request[i]);
  }
  MPI_Waitall(workers, request, MPI_STATUS_IGNORE);
  for (i = 0; i<workers;i++){
    MPI_Isend(&task[i+workers], 1, MPI_INT, i+1, i+workers , MPI_COMM_WORLD , &request[i]);
  }
  //int index;
  i=2*workers;
  int indices[3];
  ...
  while (i < MAX_TASKS)
  {
    ...
    for (int j = 0; j<index_count;j++){
      ...
      MPI_Irecv(&temp, 1, MPI_INT, indices[j]+1, MPI_ANY_TAG, MPI_COMM_WORLD, &recived_request[indices[j]]);
      //MPI_Wait(&request[indices[j]] , MPI_STATUS_IGNORE);
    ...
    MPI_Irecv(&temp, 1, MPI_INT, index+1, MPI_ANY_TAG, MPI_COMM_WORLD, &recived_request[index]);
    
    //MPI_Wait(&request[index] , MPI_STATUS_IGNORE);
    MPI_Isend(&task[i], 1, MPI_INT, index+1, i, MPI_COMM_WORLD, &request[index]);
    printf("time %d , who %d||", task[i],index+1);
    i++;*/
  }
  printf("Wait\n");
  MPI_Waitall(workers, request , MPI_STATUS_IGNORE);
