...
void master (int nworker, Data& ds) {
  ...
  for (i=0; i<nworker; i++) {
     MPI_Send(&settings[i][0], 8, MPI_DOUBLE, i+1, i, MPI_COMM_WORLD);
   }  

   while (i<n_settings) {
     MPI_Recv(&temp, 1, MPI_DOUBLE, MPI_ANY_SOURCE, MPI_ANY_TAG, 
                                 MPI_COMM_WORLD, &status);
     who = status.MPI_SOURCE;
     tag = status.MPI_TAG;
     accuracy[tag] = temp;
     MPI_Send(&settings[i][0], 8, MPI_DOUBLE, who, i, MPI_COMM_WORLD);
     i++;
   }
   ...
   for (i=0; i<nworker; i++) {
     ...
     tag = status.MPI_TAG;
     accuracy[tag] = temp;
     MPI_Send(&settings[i][0], 8, MPI_DOUBLE, who, NO_MORE_TASKS, 
                                            MPI_COMM_WORLD);
   }  
...
{
   double result;
   std::array<double,8> task;
   MPI_Status status;

   MPI_Recv(&task, 8, MPI_DOUBLE, 0, MPI_ANY_TAG, MPI_COMM_WORLD,
                                                      &status);
